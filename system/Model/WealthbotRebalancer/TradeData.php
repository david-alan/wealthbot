...
class TradeData extends Base {
    ...
    {
        if (0 === strcasecmp($action, QueueItem::STATUS_SELL)) {
            $action = self::ACTION_SELL;
        } elseif (0 === strcasecmp($action, QueueItem::STATUS_BUY)) {
    ...
    public function toArrayForTradeFile()
    {
        return array(
            ...
            'F' => ($this->getQuantityType() === self::QUANTITY_TYPE_ALL_SHARES ? '' : $this->getQuantity()),
            'G' => $this->getSymbol(),
