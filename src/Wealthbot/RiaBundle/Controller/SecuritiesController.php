...
class SecuritiesController extends BaseController
{
    public function modelSecuritiesListAction(Request $request)
    {
        ...
        return $this->render('WealthbotRiaBundle:Securities:model_securities_list.html.twig', array(
    ...
    public function completeSubclassesAction(Request $request)
    {
        ...
        if(!$assetClass) throw $this->createNotFoundException(sprintf("AssetClass with id %d does not exist.", $request->get('asset_id')));

        $subclasses = $em->getRepository('WealthbotAdminBundle:Subclass')->findBy(array('asset_class_id' => $assetClass->getId(), 'owner_id' => $ria->getId()));
    ...
    public function deleteModelSecurityAction(Request $request)
    {
        ...
        if(!$securityAssignment) throw $this->createNotFoundException(sprintf("SecurityAssignment with id %d does not exist.", $request->get('id')));
        ...
        return $this->redirect($this->generateUrl('rx_ria_model_securities_list', array('model_id' => $model->getId())));
    }
    ...
    public function editModelSecurityAction(Request $request)
    {
        ...
        return $this->render('WealthbotRiaBundle:Securities:model_securities_list_edit.html.twig', array(
            'form'       => $form->createView(),
            'is_show_municipal_bond' => $riaCompanyInformation->getUseMunicipalBond(),
        ...
        if($request->isMethod('post')) {
            ...
            if($form->isValid()) {
                ...
                return $this->getJsonResponse(array(
                    ...
                    'content' => $this->renderView('WealthbotRiaBundle:Securities:security_transaction_edit.html.twig', array(
                        'form' => $form->createView(),
