...
class DoctrineSubscriberTest extends ExtendedWebTestCase
{
    ...
    {
        $this->riaUser = $this->authenticateUser('ria', array('ROLE_RIA', 'ROLE_RIA_BASE', 'ROLE_ADMIN'));

        $specs = $this->em->getRepository('WealthbotAdminBundle:BillingSpec')->findBy(array('owner' => $this->riaUser));
        $message = "\n ALL SPECS BEFORE:\n";
        ...
        $spec1 = $this->em->getRepository('WealthbotAdminBundle:BillingSpec')->findOneBy(array('name' => 'Flat spec'));
        $spec2 = $this->em->getRepository('WealthbotAdminBundle:BillingSpec')->findOneBy(array('name' => 'Tier spec'));
        ...
        $this->assertEquals(true, $spec3->getMaster(), 'TestSpec3 must be a master. ' . $message);

        $this->assertNotNull($spec2, 'was not loaded Tier spec');
        $this->assertEquals(false, $spec2->getMaster(), 'Tier spec must NOT be a master. ' . $message);

        $this->assertNotNull($spec1, 'was not loaded Flat spec');
        $this->assertEquals(false, $spec1->getMaster(), 'Flat spec must NOT be a master. ' . $message);
    }
    ...
    public function testSetMaster()
    {
        ...
        $this->assertNotNull($spec2, 'was not loaded Tier spec');
        $this->assertEquals(true, $spec2->getMaster(), 'spec2 must be a master');

        $this->assertNotNull($spec1, 'was not loaded Flat spec');
        $this->assertEquals(false, $spec1->getMaster(), 'spec1 must NOT be a master');
    }
    ...
    public function testNoMaster()
    {
        ...
        $this->assertNotNull($spec1, 'was not loaded Flat spec');
        $this->assertEquals(true, $spec1->getMaster(), 'spec1 must be a master');

        $this->assertNotNull($spec2, 'was not loaded Tier spec');
        $this->assertEquals(false, $spec2->getMaster(), 'spec2 must NOT be a master');
    }
    ...
    public function testAddThreeMasters()
    {
        ...
        $this->assertNotNull($spec3, 'was not created TestSpec3');
        $this->assertEquals(true, $spec3->getMaster(), 'TestSpec3 must be a master');

        $this->assertNotNull($spec2, 'was not loaded Tier spec');
        $this->assertEquals(false, $spec2->getMaster(), 'Tier spec must NOT be a master');

        $this->assertNotNull($spec1, 'was not loaded Flat spec');
        $this->assertEquals(false, $spec1->getMaster(), 'Flat spec must NOT be a master');
    }
    ...
    public function testAdminFee()
    {
        ...
        $this->assertEquals(false, $specB->getMaster(), 'TestAdminSpecB must NOT be a master');

        $this->assertNotNull($specA, 'was not loaded Tier spec');
        $this->assertEquals(true, $specA->getMaster(), 'TestAdminSpecA must be a master');

        $this->assertNotNull($spec1, 'was not loaded TestSpec1');
        $this->assertEquals(true, $spec1->getMaster(), 'TestSpec1 must be a master');
    }
