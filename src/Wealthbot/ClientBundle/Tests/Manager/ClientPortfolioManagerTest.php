...
class ClientPortfolioManagerTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function setUp()
    {
        ...
        $this->repository = $this->getMockBuilder('Doctrine\Common\Persistence\ObjectRepository')
            ->setMethods(array('find', 'findAll', 'findOneBy', 'findBy', 'getClassName'))
            ->getMock();
        ...
        $this->repository->expects($this->any())
            ...
            ->will($this->returnCallback(array($this, 'getMockPortfolios')));
    ...
    public function testProposePortfolio()
    {
        ...
        $this->assertEquals($this->getMockClient(), $proposedPortfolio->getClient(), 'Invalid client object for proposed portfolio.');
        $this->assertEquals($this->getMockModel(), $proposedPortfolio->getPortfolio(), 'Invalid portfolio object for proposed portfolio.');
        $this->assertEquals(ClientPortfolio::STATUS_PROPOSED, $proposedPortfolio->getStatus(), 'Invalid status of proposed portfolio.');
    }
    ...
    public function testApproveProposedPortfolio()
    {
        ...
        $this->repository->expects($this->at(1))
            ...
            ->will($this->returnCallback(array($this, 'getMockPortfolio')));

        $approvedPortfolio = $this->manager->approveProposedPortfolio($client);

        $this->assertEquals($this->getMockClient(), $approvedPortfolio->getClient(), 'Invalid client object for approved portfolio.');
        $this->assertEquals($this->getMockModel(), $approvedPortfolio->getPortfolio(), 'Invalid portfolio object for approved portfolio.');
        $this->assertEquals(ClientPortfolio::STATUS_ADVISOR_APPROVED, $approvedPortfolio->getStatus(), 'Invalid status of approved portfolio.');
    ...
    public function testAcceptApprovedPortfolio()
    {
        ...
        $this->assertEquals($this->getMockClient(), $acceptedPortfolio->getClient(), 'Invalid client object for accepted portfolio.');
        $this->assertNotNull($acceptedPortfolio->getPortfolio()->getId(), 'Invalid portfolio object for accepted portfolio.');
        $this->assertEquals(ClientPortfolio::STATUS_CLIENT_ACCEPTED, $acceptedPortfolio->getStatus(), 'Invalid status of accepted portfolio.');
        $this->assertNotNull($acceptedPortfolio->getAcceptedAt(), 'Invalid portfolio accepted date.');
        $this->assertEquals(true, $acceptedPortfolio->getIsActive(), 'Invalid is_active flag value of approved portfolio.');
    }
    ...
    private function getMockClient()
    {
        ...
        $mockClient->setRoles(array('ROLE_USER', 'ROLE_CLIENT'));
    ...
    {
        $mockModel = $this->getMock('Wealthbot\AdminBundle\Entity\CeModel', array('getId'));
    ...
    {
        return array($this->getMockPortfolio($params));
    }
}
